...
- [ ] 2.2 Verify each invalidates: `ACADEMIC_YEARS_QUERY_KEY`,
      `[ACADEMIC_TERMS_QUERY_KEY, yearId]`,
      `[CURRICULUM_VERSIONS_QUERY_KEY, yearId]` as appropriate.
- [ ] 2.3 Fill any gaps. (Coordinate with Cluster B task 6 if both are
      in-flight — avoid duplicate invalidation logic.)
- [ ] 2.4 Manual test: new tenant → create year → activate → scope
...
- [ ] 3.1 In `settings/school-profile/page.tsx`, extract a `SchoolProfileView`
...
- [ ] 3.2 Add an "Edit" button in the Card header (alongside the title).
- [ ] 3.3 Wrap the existing `SchoolProfileForm` in a `Dialog` that opens on
      "Edit" click. The form remains unchanged.
- [ ] 3.4 On successful submit: close the dialog, invalidate
      `SCHOOL_PROFILE_QUERY_KEY`, show success toast. The view-mode refetches
      and shows the updated data.
- [ ] 3.5 Add a "Batal" button in the dialog footer that closes without
      saving.
- [ ] 3.6 Logo upload: keep as-is (it's a separate section, not part of the
...
- [ ] 4.2 End-to-end: new tenant → dashboard empty state → create year →
