# web-onboarding-ui Specification

## Purpose

Defines onboarding-related web console UI behavior: persisting and restoring
the user's academic scope selection across reloads, presenting the school
profile in a view-with-edit-modal pattern, and keeping scope-dependent
queries in sync after scope-affecting mutations.
...
Create, transition, and delete mutations for academic years, terms, and curriculum versions MUST invalidate the query keys that `AcademicScopeProvider` depends on, so the scope selectors populate without requiring a manual page reload.
