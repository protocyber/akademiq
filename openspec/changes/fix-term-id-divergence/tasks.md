...
- [ ] 1.4 Unit/integration test: write with real Active `term_id` succeeds;
...
- [ ] 2.3 Integration test: republish enqueues one event per existing term with
      the real `term_id`.
...
- [ ] 3.5 Integration test: after republish, reconcile remaps evaluation/
      report_type, deletes ghost `valid_term` rows; second run reports no-change.

## 4. End-to-end heal verification

- [ ] 4.1 On a seeded environment, run republish → wait for `valid_term` real
      rows → run reconcile.
- [ ] 4.2 Reproduce the reported bug: POST `/grading/evaluations` with the real
      `term_id` of an Active term returns 201 (was `TERM_NOT_EDITABLE`).
- [ ] 4.3 Confirm the report board lists existing report types for the year (no
      "Belum ada jenis rapor untuk tahun ini").
- [ ] 4.4 Confirm `valid_term` has no `md5(year)` rows and
      `evaluation`/`report_type` reference real `term_id`s.

## 5. Docs & rollout

- [ ] 5.1 Document the ordered heal (republish → reconcile) in the backend
...
- [ ] 5.2 Note the rollback approach (retain old→new id mapping for reverse map).
- [ ] 5.3 `make test` (grading + academic-config) green.
